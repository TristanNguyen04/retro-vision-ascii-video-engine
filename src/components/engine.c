...
typedef enum {
    ...
    ENGINE_STATE_COLLECT_STAT,
    ENGINE_STATE_PREPARE_COMPRESSION,
...
typedef struct {
    EngineConfig config;
    WavFile wav;
    RenderCompressContext render_compress_ctx;

    FILE *render_fp;
    FILE *compress_render_fp;
    FILE *log_fp;
...
static EngineError engine_prepare_compression(EngineContext *ctx);
static EngineError engine_collect_stats(EngineContext *ctx); /* collect freq for huffman compression */
    ...
    while (ctx.state != ENGINE_STATE_DONE && ctx.state != ENGINE_STATE_ERROR) {
        ...
        case ENGINE_STATE_PREPARE_SEQUENCE:
            ...
            if (ctx.error != ENGINE_OK) {
                ...
                ctx.state = ENGINE_STATE_COLLECT_STAT;
            }
            break;

        case ENGINE_STATE_COLLECT_STAT:
            ctx.error = engine_collect_stats(&ctx);

            if (ctx.error != ENGINE_OK) {
                ctx.state = ENGINE_STATE_ERROR;
            } else {
                ctx.state = ENGINE_STATE_PREPARE_COMPRESSION;
            }
            break;

        case ENGINE_STATE_PREPARE_COMPRESSION:
            ctx.error = engine_prepare_compression(&ctx);
        ...
        case ENGINE_STATE_OPEN_OUTPUTS:
            ctx.error = engine_open_outputs(&ctx, render_path, render_compress_path, log_path);
...
static void engine_init(EngineContext *ctx) {
    ...
    wav_init(&ctx->wav);
    render_compress_ctx_init(&ctx->render_compress_ctx);
...
static void engine_cleanup(EngineContext *ctx) {
    ...
    wav_free(&ctx->wav);
    render_compress_ctx_free(&ctx->render_compress_ctx);
    config_init(&ctx->config);
}
...
static int engine_log_wav(EngineContext *ctx) {
    ...
    return fprintf(
               ...
               ctx->wav.data_size,
               ctx->wav.sample_count_per_channel) >= 0;
}
...
static EngineError engine_open_outputs(
    ...
    const char *render_compress_path,
    const char *log_path) {
    if (ctx == NULL || render_path == NULL || render_compress_path == NULL || log_path == NULL) {
        return ENGINE_ERR_NULL_ARG;
    }
    ...
    /* should i move to new state instead of this open state? */
    render_write_header(ctx->compress_render_fp, &ctx->render_compress_ctx);

    ctx->log_fp = fopen(log_path, "w");
    if (ctx->log_fp == NULL) {
        fclose(ctx->render_fp);
        fclose(ctx->compress_render_fp);
        ctx->render_fp = NULL;
        ctx->compress_render_fp = NULL;
...
static EngineError engine_collect_stats(EngineContext *ctx) {
    unsigned int frame;
    char frame_path[512];
    ...
    memset(ctx->render_compress_ctx.symbol_freq, 0, sizeof(ctx->render_compress_ctx.symbol_freq));

    for (frame = ctx->config.start_frame;
         frame <= ctx->config.end_frame;
         frame++) {

        if (sequence_build_frame_path(
                &ctx->config,
                frame,
                frame_path,
                sizeof(frame_path)) != SEQUENCE_OK) {
            return ENGINE_ERR_SEQUENCE;
        }
        ...
        if (bmp_load(frame_path, &bmp) != BMP_OK) {
            bmp_free(&bmp);
            ascii_free(&ascii_frame);
            return ENGINE_ERR_BMP;
        }

        if (ascii_render_image_with_highlight(
                &bmp,
                ctx->config.palette,
                0,
                &ascii_frame) != ASCII_OK) {
        ...
        if (!flat) {
            bmp_free(&bmp);
            ascii_free(&ascii_frame);
            return ENGINE_ERR_INTERNAL;
        }

        for (i = 0; flat[i]; i++) {
            ctx->render_compress_ctx.symbol_freq[(unsigned char)flat[i]]++;
        }
...
static EngineError engine_prepare_compression(EngineContext *ctx) {
    ...
    } else if (strcmp(ctx->config.compress_algorithm, "rle") == 0) {
        ctx->render_compress_ctx.compression = COMPRESS_RLE;
    ...
    if (ctx->render_compress_ctx.compression != COMPRESS_HUFFMAN)
        return ENGINE_OK;

    ctx->render_compress_ctx.huffman_K = ctx->config.huffman_K;
    ...
    /* Build Huffman tree */
    root = huffman_build_tree(
        symbols,
        ctx->render_compress_ctx.symbol_freq,
        256);
    if (!root)
        return ENGINE_ERR_INTERNAL;

    /* Generate code table */
    huffman_generate_codes(root, table);

    for (i = 0; i < 256; i++) {
        ctx->render_compress_ctx.codes[i] = table[i].bits;
        ctx->render_compress_ctx.code_lengths[i] = table[i].length;
    }

    ctx->render_compress_ctx.fsm =
        huffman_build_fsm(root, ctx->config.huffman_K);
...
static EngineError engine_process_one_frame(EngineContext *ctx) {
    ...
    RenderError render_compress_err;
    ...
    compressed_frame = compress_frame(&ctx->render_compress_ctx, &ascii_frame);
    ...
    compressed_frame.frame_number = ctx->current_frame;
    compressed_frame.timestamp = timestamp;
    compressed_frame.highlight = highlight;

    render_compress_err = render_write_frame_compressed(ctx->compress_render_fp, &compressed_frame);
    if (render_compress_err != RENDER_OK) {
        bmp_free(&bmp);
        ascii_free(&ascii_frame);
        render_compress_ctx_free(&ctx->render_compress_ctx);
        return ENGINE_ERR_RENDER_WRITE;
